...
SELECT *
FROM vendor v
JOIN vendor_booth_assignments vb
...
SELECT vendor_id, COUNT (booth_number) AS count_of_booth
...
SELECT cp. customer_id, c.customer_first_name, c.customer_last_name, SUM (quantity * cost_to_customer_per_qty) as total_spent
FROM customer c
...
FROM customer_purchases
WHERE STRFTIME('%Y', market_date)  = '2023' AND STRFTIME('%m',market_date) = '04' 
